...
function MetricSubtext({ children }: { children: ReactNode }) {
  return <div className="mt-1 text-sm font-medium text-muted-foreground/85">{children}</div>;
}
  ...
  return (
    <div className="flex h-full flex-col gap-4 py-1">
      ...
      <div>
        ...
        <MetricSubtext>среднее, дн.</MetricSubtext>
      </div>
  ...
  return (
    <div className="flex h-full flex-col gap-3 py-1">
      ...
      <div>
        ...
        <MetricSubtext>задач в работе</MetricSubtext>
      </div>
      {total > 0 && (
...
function CompositeCard({ title, children, className, layout = 'split' }: CompositeCardProps) {
  ...
  return (
    <SectionCard
      title={title}
      className={className}
      titleClassName="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground"
    >
      <div className={stacked ? 'grid grid-cols-1 gap-3' : 'grid grid-cols-1 gap-4 md:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] md:items-stretch md:gap-5'}>
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        ...
        <div>
          <CompositeCard title="Время в потоке (Cycle Time)" className="h-full">
            <TimeSlice
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(320px,0.9fr)_minmax(0,1.1fr)]">
        <div className="flex h-full flex-col">
          <MetricPanel title="Скорость завершения (Throughput)" tooltip="Производительность команды: сколько задач завершается за неделю. Стабильный Throughput — признак предсказуемого потока. Используйте для прогнозирования сроков." className="flex h-full flex-col justify-between">
            <div className="flex h-full flex-col">
              <div className="text-[38px] font-semibold leading-none tracking-tight text-foreground tabular-nums">{fmtNum(mean(tpValues))}</div>
              <MetricSubtext>среднее задач / нед.</MetricSubtext>
              {(() => {
        ...
        <CompositeCard title="Работа в процессе (WIP)" className="h-full">
          <WipSlice
